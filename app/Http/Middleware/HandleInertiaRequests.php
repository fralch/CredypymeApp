...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request)
    {
        ...
        if (session('usuario_dni') == null) {
            ...
            $agencias_verificar = (array)[];

            if ($enviroment == 'production') {
                $agencias_verificar = (array)[1, 4, 6];
            }

            $dispositivo = session('dispositivo');

            // Filtro de SEGURIDAD para acceso por COMPUTADORA 
            if ($dispositivo->tipo == 'COMPUTADORA' && in_array(session('id_agencia'), $agencias_verificar)) {
                $dispositivo = Dispositivo::where([
                    ['dispositivo', session('dispositivo')->nombre],
                    ['habilitado', 1]
                ])->get()->last();
            }

...
use App\Models\Creditos\Cuenta\TipoMovimiento;
...
class CajaTransferenciaController extends Controller
{
    public function transferencia($tipo)
    {
        ...
        if (empty($x['usuario_dni'])) {
            ...
            if ($tipo == 'A_CAJA') {
                $band = (new PermisosController)->verificarPermiso($x['usuario_dni'], 'TRANSFERENCIA_CAJA', 'CREDITOS_CAJA');
            } else if ($tipo == 'A_CUENTA') {
                $band = (new PermisosController)->verificarPermiso($x['usuario_dni'], 'TRANSFERENCIA_CUENTA', 'CREDITOS_CAJA');
            }
            ...
            if ($band == 1) {
                ...
                return Inertia::render('Creditos/Caja/transferencia', [
    ...
    public function registrar(Request $request)
    {
        ...
        CajaTransferencia::on($conexion)->create([
            'tipo' => $tipo,
    ...
    public function mis_transferencias()
    {
        ...
        if (empty($x['usuario_dni'])) {
            ...
            $band = (new PermisosController)->verificarPermiso($x['usuario_dni'], 'MIS_TRANSFERENCIAS', 'CREDITOS_CAJA');
            ...
            if ($band == 1) {
                ...
                $tu_caja = Caja::on($conexion)->where([
                ...
                if ($tu_caja != null) {
                    ...
                    $transferencias_de_caja = CajaTransferencia::on($conexion)->from('caja_transferencias as caj_tra')
                        ...
                        ->join('solucion_master.usuarios as us_2', 'caj_reg_2.dni', 'us_2.dni')
                        ->where([
                            ['caj_tra.tipo', 'A_CAJA'],
                            ['caj_tra.destinatario_id', $tu_caja_id],
                    ...
                    $transferencias_recibidas = CuentaTransferencia::on($conexion)->from('cuenta_transferencias as cue_tra')
                        ...
                        ->join('caja_registros as caj_reg_1', 'cue_tra.destinatario_id', 'caj_reg_1.id')
                        ->join('solucion_master.usuarios as us_1', 'cue_usu.dni', 'us_1.dni')
                        ->join('solucion_master.usuarios as us_2', 'caj_reg_1.dni', 'us_2.dni')
                        ->where([
                            ['cue_tra.tipo', 'A_CAJA'],
                            ['cue_tra.destinatario_id', $tu_caja_id],
                    ...
                    $transferencias_a_caja = CajaTransferencia::on($conexion)->from('caja_transferencias as caj_tra')
                        ->select(
                            'caj_tra.id',
                            'caj_tra.tipo',
                            'caj_tra.tipo as tipo_envio',
                            'caj_tra.remitente_id',
                            'caj_tra.destinatario_id',
                            'caj_tra.descripcion',
                            'caj_tra.monto',
                        ...
                        ->where([
                            ['caj_tra.tipo', 'A_CAJA'],
                            ['caj_tra.remitente_id', $tu_caja_id],
                            [DB::raw("SUBSTR(caj_tra.datos_creacion,11,10)"),  $fecha_actual]
                        ]);


                    $transferencias_enviadas = CajaTransferencia::on($conexion)->from('caja_transferencias as caj_tra')
                        ->select(
                            'caj_tra.id',
                            'caj_tra.tipo',
                            'caj_tra.tipo as tipo_envio',
                ...
                return Inertia::render('Creditos/Caja/mis_transferencias', [
    ...
    public function confirmar(Request $request)
    {
        ...
        if ($modo == 'ACEPTAR') {
            ...
            if ($tipo_envio == 'DE_CAJA') {
                ...
                $tipo_movimiento = TipoMovimiento::on($conexion)->where('nombre', 'TRANSFERENCIA A CAJA')->get()->last();
        ...
        return redirect()->route('caj.mis_transferencias');
    }
}

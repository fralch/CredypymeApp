										...
										<div class="form-row">
											...
											<div class="input-group col-md-3 mb-1 mt-1">
												...
												<input
													...
													:value="
														Object.keys(datos_credito).length > 0
															? datos_credito.codigo_expediente
															: 0
													"
													onkeydown="return false"
													spellcheck="false"
...
export default {
	...
	data() {
		return {
			datos_credito: {},
			datos_cuotas: null,
			creditos_vinculados_a: null,
			creditos_vinculados_de: null,
			credito_adicionales: null,
			notificaciones: null,
			usuarios_cobradores: null,
			...
			frmDatosCobranza: {
				...
				usuario_cobrador: this.datos_credito
					? this.datos_credito.pago_oficina
						? 0
	...
	computed: {
		...
		detalle_cuotas() {
			if (Object.keys(this.datos_credito).length > 0) {
				let lista = [];
	...
	methods: {
		...
		async ListarRecursos() {
			...
			await axios
				.get(api_externa + "/api/caj/cobranza_externa/informacion_credito", {
					params,
				})
				.then((response) => {
					this.datos_credito = response.data.datos_credito;
					this.datos_cuotas = response.data.datos_cuotas;
					this.creditos_vinculados_a = response.data.creditos_vinculados_a;
					this.creditos_vinculados_de = response.data.creditos_vinculados_de;
					this.credito_adicionales = response.data.credito_adicionales;
					this.notificaciones = response.data.notificaciones;
					this.usuarios_cobradores = response.data.usuarios_cobradores;
					this.cancelado = response.data.cancelado;
				});
		},
		...
		async DetallePagos() {
			...
			await axios
				.get(api_externa + "/api/caj/pago_cuotas/listar", { params })
				.then((response) => {
					pago_cuotas = response.data.pago_cuotas;
					mdlDetallePagos.pago_cuotas = pago_cuotas;
				});

			await axios
				.get(api_externa + "/api/caj/pago_moras/listar", { params })
				.then((response) => {
					pago_moras = response.data.pago_moras;
					mdlDetallePagos.pago_moras = pago_moras;
				});

			await axios
				.get(api_externa + "/api/caj/pago_notificaciones/listar", { params })
				.then((response) => {
					pago_notificaciones = response.data.pago_notificaciones;
		...
		async PagarCredito() {
			...
			if (
				...
				Swal.fire({
					...
					html:
						'<label class="p-2 text-white bg-success" style="font-weight: bolder;">' +
						mensaje +
					...
					if (result.isConfirmed) {
						...
						data.append("agencia_origen", this.datos_usuario.id_agencia);
						...
						// this.$inertia.post(route("caj.cobranza_externa.pagar"), data);
						// return false;
						...
						Swal.fire({
							...
							willOpen: async () => {
								...
								return await axios
									.post(route("caj.cobranza_externa.pagar"), data)
									.then(async (response) => {
										this.cancelado = response.data.cancelado;
		...
		async CancelarCredito() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					// this.$inertia.post(route("caj.cobranza_externa.cancelar"), data);
					// return false;
					...
					Swal.fire({
						...
						willOpen: async () => {
							...
							return await axios
								.post(route("caj.cobranza_externa.cancelar"), data)
								.then(async (response) => {
									this.cancelado = response.data.cancelado;
									this.datos_voucher = response.data.datos_voucher;

	...
	<layout ref="layout">
		...
		>
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'RECEPCIÓN DE PAGOS'"></headerClose>
					<div class="card">
						<div class="card-body card-block">
							<div class="form-row">
								<div class="form-group col-md-6">
									...
									<span
										v-if="
											submited && !$v.frmDatosEnvio.agencia_remitente_id.noZero
										"
									...
									<select
										...
										v-model="frmDatosEnvio.agencia_remitente_id"
									>
									...
									<label class="label-title">USUARIO ORIGEN</label>
									<span
										v-if="
											submited && !$v.frmDatosEnvio.usuario_remitente.noZero
										"
										class="span-error-message"
										>*</span
									>
									<select
										class="form-control center"
										v-model="frmDatosEnvio.usuario_remitente"
									>
										<option :value="0" selected disabled>Seleccionar...</option>
										<option
								...
								<div class="form-group col-md-6">
									...
									<label class="label-title">CAJA DESTINO</label>
								...
								<div class="form-group col-md-4">
									<label class="label-title">MOTIVO DE RECEPCIÓN</label>
									<span
							...
							<div class="text-right">
								...
								>
									...
									<span class="text">RECEPCIONAR</span>
								</button>
							</div>
						</div>
					</div>
...
export default {
	...
	methods: {
		...
		obtenerCajasAbiertas() {
			let self = this;
			self.frmDatosEnvio.usuario_remitente = 0;
			self.cajas_filtrados = self.usuarios.filter((item) => {
				return item.agencia_id == self.frmDatosEnvio.agencia_remitente_id;
			});
		},
		enviarPagos() {
			this.submited = true;
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					data.append("agencia_destinatario", this.mi_usuario.id_agencia);
					data.append("caja_destinatario", this.mi_caja.id);
					data.append("frmDatosEnvio", JSON.stringify(this.frmDatosEnvio));

					this.$inertia.post(route("caj.recepcion_pagos.registrar"), data, {
						preserveScroll: true,
						onStart: () => {
							Swal.fire({
								title: "REGISTRANDO",

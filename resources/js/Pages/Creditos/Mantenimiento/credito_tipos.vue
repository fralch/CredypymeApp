<template>
	<layout ref="layout">
		<div class="slot_body slot-creditos-tipos" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'TIPOS DE CRÉDITO'"></headerClose>
							...
							<div class="form-group col-md-4">
								<div class="input-group">
									...
									<select
										...
										@change="getListaTipo()"
										v-model="frmDatosTipo.agencia_seleccionada"
									>
										<option
              ...
              >
                <span class="icon text-white">
                  <i class="fas fa-plus"></i>
						...
						<table class="table table-hover" id="tblTipos" width="100%">
							<thead>
								<tr>
							...
							<tbody>
								<tr v-for="(item, index) in tipos_seleccionados" :key="index">
			...
			<!-- The Modal -->
			<div id="mdlDatosTipo" class="modal modal-right">
				<!-- Modal content -->
							...
							<div class="card-body card-block">
								<form autocomplete="off" @submit.prevent="Guardar">
									<div class="form-row">
										...
										<div class="row" v-if="frmDatosTipo.modo == 'EDITAR'">
...
export default {
	...
	data() {
		return {
			submited: false,
			agencias_permitidas: [],
			tipos_seleccionados: this.tipos,
			titulo_modal: "NUEVO TIPO",
			frmDatosTipo: {
	...
	watch: {
		agencias_permitidas(value) {
			...
			this.frmDatosTipo.agencia_seleccionada = agencia_id;
		},
	...
	methods: {
		getListaTipo() {
			let self = this;
			axios
				.post(route("man.cre_tipos.lista"), {
					id_agencia: this.frmDatosTipo.agencia_seleccionada,
				})
				.then(function (response) {
					self.tipos_seleccionados = response.data;
				});
		},
			...
			if (this.$v.frmDatosTipo.$invalid) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						axios
							.post(route("man.cre_tipos.verificar"), self.frmDatosTipo)
							.then(function (response) {
								if (response.data == "EXISTE") {
									Swal.fire({
										icon: "error",
										title: "¡Ups!",
										text: "Este TIPO ya existe",
									});
									return false;
								} else {
									self.$inertia.post(
										route("man.cre_tipos.guardar"),
										self.frmDatosTipo,
										{
											preserveScroll: true,
											onStart: (visit) => {

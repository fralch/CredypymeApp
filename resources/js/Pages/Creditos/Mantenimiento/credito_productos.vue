	...
	<layout ref="layout">
		<div class="slot_body slot-creditos-productos" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'PRODUCTOS DE CRÉDITO'"></headerClose>
					...
					<div class="card-body card-block">
							...
							<div class="form-group col-md-4">
								<div class="input-group">
									...
									<select
										...
										@change="getListaProductos()"
										v-model="frmDatosProducto.agencia_seleccionada"
									>
										<option
						...
						<table class="table table-hover" id="tblProductos" width="100%">
							<thead>
								<tr>
							...
							<tbody>
								<tr
									v-for="(item, index) in productos_seleccionados"
									:key="index"
								>
			...
			<!-- The Modal -->
			<div id="mdlDatosProducto" class="modal modal-right">
				<!-- Modal content -->
				<div class="modal-content w-35 mdlDatosProducto">
					<div class="content" style="display: block">
						<div class="card">
							<headerCloseModal
								:titulo_modal="titulo_modal"
								:nombre_modal="'mdlDatosProducto'"
							>
							</headerCloseModal>
								...
								<form autocomplete="off" @submit.prevent="Guardar">
									<div class="form-row">
										<div class="form-group col-md-6 col-6">
											...
											<span
												v-if="
													submited && !$v.frmDatosProducto.producto.required
												"
										...
										<div class="row" v-if="frmDatosProducto.modo == 'EDITAR'">
...
export default {
	...
	data() {
		return {
			submited: false,
			agencias_permitidas: [],
			productos_seleccionados: this.productos,
	...
	watch: {
		agencias_permitidas(value) {
			...
			this.frmDatosProducto.agencia_seleccionada = agencia_id;
		},
	...
	methods: {
		getListaProductos() {
			let self = this;
			axios
				.post(route("man.cre_productos.listar_productos"), {
					id_agencia: this.frmDatosProducto.agencia_seleccionada,
				})
				.then(function (response) {
					self.productos_seleccionados = response.data;
				});
		},
		...
		Nuevo() {
			this.submited = false;
			this.titulo_modal = "NUEVO PRODUCTO";
			this.frmDatosProducto.modo = "NUEVO";
			this.frmDatosProducto.id = 0;
			this.frmDatosProducto.producto = null;
			this.frmDatosProducto.descripcion = null;
			this.frmDatosProducto.habilitado = true;

			$("#mdlDatosProducto").css("display", "block");
		},
		...
		Editar(item) {
			...
			this.frmDatosProducto.descripcion = item.descripcion;
		...
		Guardar() {
			let self = this;
			this.submited = true;
			if (this.$v.frmDatosProducto.$invalid) {
				Swal.fire({
					icon: "error",
					title: "¡Ups!",
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						axios
							.post(route("man.cre_productos.verificar"), self.frmDatosProducto)
							.then(function (response) {
								if (response.data == "EXISTE") {
									...
									self.$inertia.post(
										route("man.cre_productos.guardar"),
										self.frmDatosProducto,
										{
											preserveScroll: true,
											onStart: (visit) => {
												let timerInterval;
											...
											onSuccess: () => {
												Swal.fire({
													...
													preConfirm: (result) => {
														self.submited = false;
														$("#mdlDatosProducto").css("display", "none");
														self.getListaProductos();
													},
												});
											},
										}

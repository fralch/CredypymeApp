<template>
	<layout ref="layout">
		<div class="slot_body slot-creditos-estados" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'ESTADOS DE CRÉDITO'"></headerClose>
							...
							<div class="form-group col-md-4">
								<div class="input-group">
									...
									<select
										...
										@change="getListaEstados()"
										v-model="frmDatosEstado.agencia_seleccionada"
									>
										<option
                ...
                <span class="icon text-white">
                  <i class="fas fa-plus"></i>
						...
						<table class="table table-hover" id="tblEstados" width="100%">
							...
							<tbody>
								<tr v-for="(item, index) in estados_seleccionados" :key="index">
...
export default {
	...
	data() {
		return {
			submited: false,
			agencias_permitidas: [],
			estados_seleccionados: this.estados,
	...
	watch: {
		agencias_permitidas(value) {
			...
			this.frmDatosEstado.agencia_seleccionada = agencia_id;
		},
	...
	methods: {
		getListaEstados() {
			let self = this;
			axios
				.post(route("man.cre_estados.listar"), {
					id_agencia: this.frmDatosEstado.agencia_seleccionada,
				})
				.then(function (response) {
					self.estados_seleccionados = response.data;
				});
		},
			...
			if (this.$v.frmDatosEstado.$invalid) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						axios
							.post(route("man.cre_estados.verificar"), self.frmDatosEstado)
							.then(function (response) {

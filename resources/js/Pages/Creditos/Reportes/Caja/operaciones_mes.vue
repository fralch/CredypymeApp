<template>
	<layout ref="layout">
		<div class="slot_body slot-reporte-operaciones-mes" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'OPERACIONES DEL MES'"></headerClose>
...
export default {
	components: { layout, headerClose, rptCierreCaja, DatePicker },
	props: {
		usuarios: Array,
	},
	data() {
	...
	methods: {
		...
		Buscar() {
			...
			Swal.fire({
				...
				didOpen: () => {
					Swal.showLoading();
					//   this.$inertia.post(route("rep.caj.operaciones_mes.buscar"), data);
					//   return false
					axios
						.post(route("rep.caj.operaciones_mes.buscar"), data)
		...
		Exportar() {
			...
			Swal.fire({
				...
				didOpen: () => {
					...
					axios
						.post(route("rep.caj.operaciones_mes.exportar"), data)
						.then(function (response) {
							let path_xlsx = response.data.path_xlsx;

							const link = document.createElement("a");
							link.href = origin + path_xlsx;
							link.download = "rptOperacionesMes.xlsx";
							link.click();
							return Swal.fire({
								icon: "success",
								title: "¡EXPORTADO!",

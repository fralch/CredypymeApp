					...
					<div class="card-body card-block">
						<div class="form-row">
							<div class="col-md-8">
								...
								<span
									v-if="submited && !$v.frmDatosNotificacion.tipo_id.noZero"
								...
								>
									...
									<option
										v-for="(item, index) in notificaciones_tipos"
										:key="index"
										:value="item.id"
									>
										{{ item.tipo }}
							...
							<div class="col-md-4">
								...
								<span
									v-if="
										submited &&
										(!$v.frmDatosNotificacion.monto.noZero ||
											!$v.frmDatosNotificacion.monto.required)
									"
									class="span-error-message"
								>
									*
								...
								<input
									...
									step="1"
									@change="Redondear"
									style="font-size: 1.1rem; color: var(--colorAlto)"
									v-model.number="frmDatosNotificacion.monto"
									:disabled="frmDatosNotificacion.tipo_id == 0"
								/>
							</div>
						</div>
...
export default {
	...
	watch: {
		tipo(value) {
			if (value != 0) {
				let monto = this.notificaciones_tipos.filter(
					(item) => item.id == value
				)[0].monto;

				this.monto_minimo = parseFloat(monto);
				this.frmDatosNotificacion.monto = this.roundTo(monto, 2);
			}
		},
	},
				...
				if (result.isConfirmed) {
					...
					Swal.fire({
						...
						didOpen: () => {
							...
							axios
								.post(route("rep.cre.dias_mora.aplicar_notificacion"), data)
								.then(function (response) {
									...
									$("#mdlNuevaNotificacion").css("display", "none");

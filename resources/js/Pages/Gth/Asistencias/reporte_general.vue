<template>
	<layout ref="layout">
		<div class="slot_body slot-reporte-general" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'REPORTE GENERAL'"></headerClose>

					<div class="card-title">PANEL DE BUSQUEDA</div>
					<div class="card-body card-block">
						<div class="form-row">
							<fieldset class="form-group col-md-6">
								<legend>
									<label class="label-title">FILTROS DE BÚSQUEDA</label>
								</legend>
								<div class="row">
									<div class="input-group col-md-6">
										<div class="input-group-prepend">
											<span class="input-group-text prepend-title">DESDE</span>
										</div>
										<input
											class="form-control center"
											type="date"
											name="desde"
											id="dtpDesde"
											style="font-size: 15px"
										/>
									</div>
									<div class="input-group col-md-6">
										<div class="input-group-prepend">
											<span class="input-group-text prepend-title">HASTA</span>
										</div>
										<input
											class="form-control center"
											type="date"
											name="hasta"
											id="dtpHasta"
											style="font-size: 15px"
										/>
									</div>
								</div>
							</fieldset>

							<div class="col-md-1">
								<button
									class="btn btn-action btn-icon-split mt-3"
									@click="GenerarReporte"
								>
									<span class="icon text-white" style="font-size: 25px">
										<i class="fas fa-search"></i>
									</span>
								</button>
							</div>
						</div>

						<div class="form-row">
							<div class="input-group col-md-3 mt-1">
								<div class="input-group-prepend">
									<span class="input-group-text prepend-title">AGENCIA</span>
								</div>
								<select
									class="form-control center"
									name="slcAgencias"
									id="slcAgencias"
									data-index="0"
									v-model="agencia_busqueda"
									:disabled="lista_reporte_general_filtrados.length == 0"
								>
									<option :value="0">TODAS</option>
									<option
										v-for="(item, index) in agencias"
										:key="index"
										:value="item.nombre"
									>
										{{ item.nombre }}
									</option>
								</select>
							</div>
							<div class="input-group col-md-3 mt-1">
								<div class="input-group-prepend">
									<span class="input-group-text prepend-title">TURNO</span>
								</div>
								<select
									class="form-control center"
									name="turnos"
									id="slcTurnos"
									v-model="turno_busqueda"
									data-index="4"
									:disabled="lista_reporte_general_filtrados.length == 0"
								>
									<option value="0">TODOS</option>
									<option value="ingreso_mañana">Turno mañana</option>
									<option value="ingreso_tarde">Turno tarde</option>
								</select>
							</div>
							<div class="input-group col-md-4 mt-1">
								<div class="input-group-prepend">
									<div class="input-group-text">
										<input
											type="checkbox"
											id="chbPorUsuario"
											v-model="por_asesor"
											:disabled="lista_reporte_general_filtrados.length == 0"
										/>
									</div>
									<label
										class="input-group-text prepend-title"
										for="chbPorUsuario"
									>
										COLABORADOR
									</label>
								</div>
								<div class="input-group-prepend"></div>
								<select
									class="form-control center"
									v-model="usuario_seleccionado"
									id="slcUsuarios"
									:disabled="!por_asesor"
									data-index="1"
								>
									<option :value="0">Seleccione...</option>
									<option
										v-for="(item, index) in usuarios_filtrados"
										:key="index"
										:value="item.dni"
									>
										{{ item.usuario }}
									</option>
								</select>
							</div>

							<div class="input-group col-md-2 mt-1 offset-7">
								<div class="form-check">
									<input
										class="form-check-input"
										type="checkbox"
										id="chbMostrarHabilitados"
										v-model="mostrar_habilitados"
										:disabled="!por_asesor"
										@change="FiltrarUsuarios"
									/>
									<label class="label-title" for="chbMostrarHabilitados"
										>Sólo habilitados</label
									>
								</div>
							</div>
						</div>

						<div class="card-title mb-1">RESULTADOS DE BÚSQUEDA</div>

						<table
							class="table table-hover"
							id="tblReporteGeneral"
							width="100%"
						>
							<thead>
								<tr>
									<th>AGENCIA</th>
									<th>DNI</th>
									<th>COLABORADOR</th>
									<th>FECHA_REGISTRO</th>
									<th>TURNO</th>
									<th>HORA_ENTRADA</th>
									<th>HORA_SALIDA</th>
									<th>MIN_TARDANZA</th>
									<th>OBSERVACIÓN</th>
									<th>JUSTIFICACIÓN</th>
								</tr>
							</thead>
							<tbody>
								<tr
									v-for="(item, index) in lista_reporte_general_filtrados"
									:key="index"
								>
									<td class="table-bordered" align="center">
										{{ item.nombre_agencia }}
									</td>
									<td class="table-bordered" align="center">
										{{ item.usuario_id }}
									</td>
									<td class="table-bordered">
										{{
											item.apellido_paterno +
											" " +
											item.apellido_materno +
											" " +
											item.nombres
										}}
									</td>
									<td class="table-bordered" align="center">
										{{ item.fecha }}
									</td>
									<td class="table-bordered" align="center">
										{{ item.turno }}
									</td>
									<td class="table-bordered" align="center">
										{{ item.hora_entrada }}
									</td>
									<td class="table-bordered" align="center">
										{{ item.hora_salida }}
									</td>
									<td class="table-bordered t-minutos" align="center">
										{{ item.minutos_tardanza }}
									</td>
									<td
										class="table-bordered"
										align="center"
										:class="[item.observacion]"
									>
										{{ item.observacion }}
									</td>
									<td class="table-bordered" align="center">
										{{ item.justificacion }}
									</td>
								</tr>
							</tbody>
						</table>
						<hr />
						<div class="text-right">
							<button
								class="btn btn-cancel btn-icon-split"
								title="Exportar"
								@click="Exportar"
								:disabled="lista_reporte_general_filtrados.length == 0"
							>
								<span class="icon text-white">
									<i class="fas fa-file-excel"></i>
								</span>
								<span class="text">EXPORTAR</span>
							</button>
						</div>
					</div>
				</div>
			</div>
		</div>
	</layout>
</template>

<script>
import layout from "@/Pages/Gth/Components/layout_gth.vue";
import headerClose from "@/Pages/Gth/Components/header_close.vue";
export default {
	components: {
		layout,
		headerClose,
	},
	props: { usuarios: Array, agencias: Array },
	data() {
		return {
			windowWidth: window.innerWidth,

			reporte_general: [],
			lista_reporte_general_filtrados: [],
			lista_reporte_general: [],

			agencia_busqueda: 0,
			turno_busqueda: 0,

			usuarios_filtrados: [],
			por_asesor: false,
			usuario_seleccionado: 0,
			mostrar_habilitados: true,

			total_minutos: 0,

			total_minutos_total: 0,
		};
	},
	mounted() {
		// window.addEventListener("resize", () => {
		//   this.windowWidth = window.innerWidth;
		//   this.AñadirResponsive();
		// });
		// this.AñadirResponsive();
		this.DiaActual();
		this.TablaReporteGeneral();
	},
	watch: {
		lista_reporte_general_filtrados() {
			$("#tblReporteGeneral").DataTable().destroy();
			this.TablaReporteGeneral();
		},
		agencia_busqueda() {
			this.FiltrarUsuarios();
		},
		por_asesor() {
			this.usuario_seleccionado = 0;
		},
	},
	methods: {
		// AñadirResponsive() {
		//   if (this.windowWidth < 1500) {
		//     if (!$("#tblReporteGeneral").hasClass("table-responsive")) {
		//       document
		//         .getElementById("tblReporteGeneral")
		//         .classList.add("table-responsive");
		//     }
		//   } else {
		//     if ($("#tblReporteGeneral").hasClass("table-responsive")) {
		//       document
		//         .getElementById("tblReporteGeneral")
		//         .classList.remove("table-responsive");
		//     }
		//   }
		// },
		hidenav() {
			return this.$refs.layout.hide_nav();
		},
		shownav() {
			return this.$refs.layout.show_nav();
		},
		roundTo(value, places) {
			if (value > 0 && value != "Infinity") {
				let power = Math.pow(10, places);
				return parseFloat(Math.round(value * power) / power).toFixed(2);
			} else {
				return 0;
			}
		},
		TablaReporteGeneral() {
			this.$nextTick(() => {
				var table = $("#tblReporteGeneral").DataTable({
					scrollY: "350px",
					scrollX: true,
					fixedColumns: {
						leftColumns: 0,
					},
					ordering: false,

					scrollCollapse: true,
					paging: false,
					fixedHeader: true,
					info: true,

					language: {
						retrieve: true,
						decimal: "",
						emptyTable: "No hay datos disponibles en la tabla",
						info: "Mostrando del _START_ al _END_ de _TOTAL_ registros",
						infoEmpty: "No se encontraron registros",
						infoFiltered: "(filtrado de _MAX_ registros)",
						infoPostFix: "",
						thousands: ",",
						lengthMenu: "Agrupar por _MENU_ filas",
						loadingRecords: "Cargando...",
						processing: "Procesando...",
						search: "Buscar:",
						zeroRecords: "No se encontraron registros",
						paginate: {
							first: "Primera",
							last: "Ultima",
							next: '<i class="fas fa-chevron-circle-right" style="font-size:20px;"></i>',
							previous:
								'<i class="fas fa-chevron-circle-left" style="font-size:20px;"></i>',
						},
						aria: {
							sortAscending: ": activar para ordenar de forma ascendente",
							sortDescending: ": activar para ordenar de forma descendente",
						},
					},
				});
				$("#slcTurnos").change(function () {
					if (this.value == 0) {
						table.column($(this).data("index")).search("").draw();
					} else {
						table.column($(this).data("index")).search(this.value).draw();
					}
					let total_minutos = 0;
					$(".t-minutos").each(function () {
						total_minutos += parseInt($(this).text());
					});
					self.total_minutos_total = total_minutos;
				});

				$("#slcUsuarios").change(function () {
					if (this.value == 0) {
						table.column($(this).data("index")).search("").draw();
					} else {
						table.column($(this).data("index")).search(this.value).draw();
					}
					let total_minutos = 0;
					$(".t-minutos").each(function () {
						total_minutos += parseInt($(this).text());
					});
					self.total_minutos_total = total_minutos;
				});

				$("#slcAgencias").change(function () {
					if (this.value == 0) {
						table.column($(this).data("index")).search("").draw();
					} else {
						table.column($(this).data("index")).search(this.value).draw();
					}
					let total_minutos = 0;
					$(".t-minutos").each(function () {
						total_minutos += parseInt($(this).text());
					});
					self.total_minutos_total = total_minutos;
				});
			});
		},

		DiaActual() {
			let fecha = new Date(); //Fecha actual
			let mes = fecha.getMonth() + 1; //obteniendo mes
			let dia = fecha.getDate(); //obteniendo dia
			let ano = fecha.getFullYear(); //obteniendo año
			if (dia < 10) dia = "0" + dia; //agrega cero si el menor de 10
			if (mes < 10) mes = "0" + mes;

			let desde = ano + "-" + mes + "-" + dia;
			let hasta = ano + "-" + mes + "-" + dia;

			$("#dtpDesde").val(desde);
			$("#dtpHasta").val(hasta);
			//   this.GenerarReporte();
		},
		FiltrarUsuarios() {
			this.usuarios_filtrados = [];
			this.usuarios_seleccionados = [];

			if (this.mostrar_habilitados) {
				if (this.agencia_busqueda == 0) {
					this.usuarios_filtrados = this.usuarios.filter(
						(item) => item.habilitado == 1
					);
				} else {
					this.usuarios_filtrados = this.usuarios.filter(
						(item) =>
							item.nombre == this.agencia_busqueda && item.habilitado == 1
					);
				}
			} else {
				if (this.agencia_busqueda == 0) {
					this.usuarios_filtrados = this.usuarios;
				} else {
					this.usuarios_filtrados = this.usuarios.filter(
						(item) => item.nombre == this.agencia_busqueda
					);
				}
			}
		},

		GenerarReporte() {
			self = this;
			let data = new FormData();
			data.append("f_desde", $("#dtpDesde").val());
			data.append("f_hasta", $("#dtpHasta").val());

			//   this.$inertia.post(route("gth.us_as.general.listar"), data);

			Swal.fire({
				title: "BUSCANDO",
				text: "Espere porfavor...",
				allowOutsideClick: false,
				didOpen: () => {
					Swal.showLoading();
					// this.$inertia.post(route("gth.us_as.general.listar"), data);
					axios
						.post(route("gth.us_as.general.listar"), data)
						.then(function (response) {
							if (response.data.length == 0) {
								self.lista_reporte_general_filtrados = [];

								self.FiltrarUsuarios();

								return Swal.fire({
									icon: "info",
									title: "¡Ups!",
									text: "No se encontraron datos",
									allowOutsideClick: true,
								});
							} else {
								self.reporte_general = response.data;

								self.FiltrarModo();
								self.FiltrarUsuarios();

								return Swal.fire({
									icon: "success",
									title: "¡Listo!",
								});
							}
						});
				},
			});
		},
		FiltrarModo() {
			this.lista_reporte_general_filtrados = this.reporte_general;
		},
		Filtrar() {
			this.lista_reporte_general = this.lista_reporte_general_filtrados;

			let id_turno = this.turno_busqueda;
			let id_asesor = this.usuario_seleccionado;
			let id_agencia = this.agencia_busqueda;

			if (this.usuario_seleccionado != 0) {
				this.lista_reporte_general = this.lista_reporte_general.filter(
					(item) => item.usuario_id == id_asesor
				);
			}

			if (id_turno != 0) {
				this.lista_reporte_general = this.lista_reporte_general.filter(
					(item) => item.turno == id_turno
				);
			}
			if (id_agencia != 0) {
				this.lista_reporte_general = this.lista_reporte_general.filter(
					(item) => item.nombre_agencia == id_agencia
				);
			}
		},
		Sumar_tardanza_filtrados() {
			var self = this;
			self.total_minutos_total = 0;
			self.lista_reporte_general_filtrados.forEach((element) => {
				self.total_minutos_total += parseInt(element.minutos_tardanza);
			});
		},
		Exportar() {
			self = this;

			self.Filtrar();

			let data = new FormData();
			data.append("datos", JSON.stringify(this.lista_reporte_general));

			data.append("total_minutos_total", this.total_minutos_total);

			Swal.fire({
				title: "EXPORTANDO",
				text: "Espere porfavor...",
				allowOutsideClick: false,
				didOpen: () => {
					Swal.showLoading();

					//   this.$inertia.post(route("gth.asi.general.exportar"), data);

					axios
						.post(route("gth.asi.general.exportar"), data)
						.then(function (response) {
							//   return Swal.fire({
							//     icon: "success",
							//     title: "¡Listo!",
							//   });

							var linkSource =
								"data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64," +
								response.data;
							var downloadLink = document.createElement("a");
							var fileName = "rptAsistenciaGeneral.xlsx";

							downloadLink.href = linkSource;
							downloadLink.download = fileName;
							downloadLink.click();

							return Swal.fire({
								icon: "success",
								title: "¡EXPORTADO!",
								timer: 2000,
								showConfirmButton: false,
							});
						});
				},
			});
		},
	},
};
</script>

<style lang="css">
.PUNTUAL {
	background-color: var(--azulOscuroEmpresarial) !important;
	color: white !important;
}
.TARDE {
	background-color: var(--colorAlto) !important;
	color: white !important;
}
.FALTA {
	background-color: var(--plomoOscuroEmpresarial) !important;
	color: white !important;
}
.TARDANZA-JUSTIFICADA {
	background-color: var(--verdeClaroEmpresarial) !important;
}
.FALTA-JUSTIFICADA {
	background-color: var(--azulClaroEmpresarial) !important;
}
.slot-reportesgenerales {
	width: 60% !important;
	margin-left: 20% !important;
}

@media (max-width: 900px) {
	.slot-reportesgenerales {
		width: 98% !important;
		margin-left: 1% !important;
	}
}
</style>

<template>
	<layout ref="layout">
		<div class="slot_body slot-historial-devoluciones" slot="component-view">
			<div class="content" style="display: block">
				<div class="card">
					<headerClose :title="'HISTORIAL DE DEVOLUCIONES'"></headerClose>
					...
					<div class="card-body card-block">
						<div class="form-row justify-content-md-center">
							...
							<div class="form-group col-md-3 col-6">
								...
								<select class="form-control center mayus" v-model="modo">
									<option value="devolucion" selected>DEVOLUCIONES</option>
									<option value="baja" selected>BAJAS</option>
								</select>
							</div>
					...
					<div class="card-body card-block">
						<div id="ModoAgrupado">
							...
							>
								...
								<tbody>
									<tr v-for="(item, index) in lista_devoluciones" :key="index">
							...
							>
								<thead>
									<tr>
										...
										<th>MEDICIÓN</th>
										<th>VALOR_UNIT(S/)</th>
										<th>DE_RESPONSABLE</th>
									</tr>
								</thead>
								<tbody>
									<tr
										v-for="(item, index) in lista_devoluciones_detalles"
										:key="index"
									>
										<td class="table-bordered" align="center">
											{{ JSON.parse(item.datos_creacion).fecha }}
										...
										<td class="table-bordered">
											{{ item.abreviacion + " - " + item.usuario_devolucion }}
										</td>
									</tr>
								</tbody>
							</table>
			...
			<!-- The Modal -->
			<div id="mdlDevolucionDetalle" class="modal">
				<div class="modal-content w-50 mdlDevolucionDetalle">
					<div class="content" style="display: block">
						<div class="card">
							<headerCloseModal
								:titulo_modal="'DETALLE DE DEVOLUCIÓN'"
								:nombre_modal="'mdlDevolucionDetalle'"
							>
							</headerCloseModal>
							<div class="card-body card-block">
								<div class="input-group row col-md-9 col-9" style="float: left">
								...
								<table class="table table-hover" id="tblDevolucionDetalle">
									<thead>
										<tr>
											<th>FECHA_DEVOLUCIÓN</th>
											<th style="min-width: 90px !important">AGENCIA</th>
									...
									<tbody>
										<tr
											v-for="(item, index) in devolucion_detalle"
											:key="index"
										>
											<td class="table-bordered" align="center">
												{{ JSON.parse(item.datos_creacion).fecha }}
								...
								<div class="text-right">
									...
									<button
										class="btn btn-action btn-icon-split"
										title="Descargar DOCUMENTO DE DEVOLUCIÓN"
										@click="Descargar(documento_devolucion)"
										:disabled="documento_devolucion == null"
									>
										<span class="icon text-white">
...
export default {
	...
	mounted() {
		...
		this.TablaDevolucionesDetalles();
	},
	watch: {
		lista_devoluciones() {
			$("#tblDevoluciones").DataTable().destroy();
			this.TablaDevoluciones();
		},
		devolucion_detalle() {
			$("#tblDevolucionDetalle").DataTable().destroy();
			this.TablaDevolucionDetalle();
		},
		lista_devoluciones_detalles() {
			$("#tblDevolucionesDetalles").DataTable().destroy();
			this.TablaDevolucionesDetalles();
		},
	},
	methods: {
		...
		TablaDevolucionDetalle() {
			this.$nextTick(() => {
				var table = $("#tblDevolucionDetalle").DataTable({
					scrollY: "350px",
					scrollX: true,
					fixedColumns: {
						leftColumns: 0,
		...
		TablaDevolucionesDetalles() {
			this.$nextTick(() => {
				var table = $("#tblDevolucionesDetalles").DataTable({
					scrollY: "350px",
					scrollX: true,
					fixedColumns: {
						leftColumns: 0,
		...
		async Buscar() {
			...
			if (
				...
				// this.$inertia.post(
				// 	route("log.sum.historial_devoluciones.buscar"),
				// 	data
				// );
				await axios
					.post(route("log.sum.historial_devoluciones.buscar"), data)
					.then((response) => {
						this.lista_devoluciones = response.data.lista_devoluciones;
						this.lista_devoluciones_detalles =
							response.data.lista_devoluciones_detalles;
					});
			}
		},
		ModoAgrupado() {
		...
		VerDetalle(item) {
			this.devolucion_detalle = this.lista_devoluciones_detalles.filter(
				(item_1) => item_1.devolucion_id == item.id
			);

			this.documento_devolucion = item.documento;

			$("#mdlDevolucionDetalle").css("display", "block");
		},
		Descargar(documento) {
			let self = this;
			let source =
				"/imagenes_server/logistica/suministros/devoluciones/" + documento + "";
			axios

<template>
  <layout ref="layout">
    <div class="slot_body slot-historial-bajas" slot="component-view">
      <div class="content" style="display: block">
        <div class="card">
          <headerClose :title="'HISTORIAL DE BAJAS'"></headerClose>
          ...
          <div class="card-body card-block">
            <div id="ModoAgrupado">
              ...
              >
                ...
                <tbody>
                  <tr v-for="(item, index) in lista_bajas" :key="index">
            ...
            <div id="ModoDetallado">
              ...
              >
                ...
                <tbody>
                  <tr
                    v-for="(item, index) in lista_bajas_detalles"
                    :key="index"
                  >
                    <td class="table-bordered" align="center">
                      {{ JSON.parse(item.datos_creacion).fecha }}
                    ...
                    <td class="table-bordered">
                      {{ item.abreviacion + " - " + item.usuario_baja }}
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <!-- The Modal -->
      <div id="mdlBajaDetalle" class="modal">
        <div class="modal-content w-50 mdlBajaDetalle">
          <div class="content" style="display: block">
            <div class="card">
              <headerCloseModal
                :titulo_modal="'DETALLE DE BAJA'"
                :nombre_modal="'mdlBajaDetalle'"
              >
              </headerCloseModal>
              <div class="card-body card-block">
                <div class="input-group row col-md-9 col-9" style="float: left">
                ...
                <table class="table table-hover" id="tblBajaDetalle">
                  <thead>
                    <tr>
                      <th>FECHA_BAJA</th>
                      <th>AGENCIA</th>
                  ...
                  <tbody>
                    <tr v-for="(item, index) in baja_detalle" :key="index">
                      <td class="table-bordered" align="center">
                        {{ JSON.parse(item.datos_creacion).fecha }}
                      </td>
                      <td class="table-bordered" align="center">
                ...
                <div class="text-right">
                  ...
                  <button
                    class="btn btn-action btn-icon-split"
                    title="Descargar DOCUMENTO DE BAJA"
                    @click="Descargar(documento_baja)"
                    :disabled="documento_baja == null"
                  >
                    <span class="icon text-white">
...
export default {
  ...
  mounted() {
    ...
    this.ModoAgrupado();
    this.TablaBajas();
    this.TablaBajasDetalles();
  },
  watch: {
    lista_bajas() {
      $("#tblBajas").DataTable().destroy();
      this.TablaBajas();
    },
    baja_detalle() {
      $("#tblBajaDetalle").DataTable().destroy();
      this.TablaBajaDetalle();
    },
    lista_bajas_detalles() {
      $("#tblBajasDetalles").DataTable().destroy();
      this.TablaBajasDetalles();
    },
  },
  methods: {
    ...
    TablaBajaDetalle() {
      this.$nextTick(() => {
        var table = $("#tblBajaDetalle").DataTable({
          scrollY: "350px",
          scrollX: true,
          fixedColumns: {
            leftColumns: 0,
    ...
    TablaBajasDetalles() {
      this.$nextTick(() => {
        var table = $("#tblBajasDetalles").DataTable({
          scrollY: "350px",
          scrollX: true,
          fixedColumns: {
            leftColumns: 0,
    ...
    Buscar() {
      ...
      if (
        ...
        // this.$inertia.post(
        //   route("log.sum.historial_bajas.buscar"),
        //   data
        // );
        axios
          .post(route("log.sum.historial_bajas.buscar"), data)
          .then(function (response) {
            self.lista_bajas = response.data.lista_bajas;
            self.lista_bajas_detalles = response.data.lista_bajas_detalles;
          });
      }
    },
    ModoAgrupado() {
    ...
    VerDetalle(item) {
      this.baja_detalle = this.lista_bajas_detalles.filter(
        (item_1) => item_1.devolucion_id == item.id
      );

      this.documento_baja = item.documento;

      $("#mdlBajaDetalle").css("display", "block");
    },
    Descargar(documento) {
      let self = this;
      let source =
        "/imagenes_server/logistica/suministros/bajas/" + documento + "";
      axios
